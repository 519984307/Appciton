...
void ConfigEditAlarmLimitMenuPrivate::loadOptions()
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // 获得子id对应的参数类型
        int type = static_cast<int>(paramInfo.getUnitOfSubParam(subID));
        // 根据子id读取xml文件中type类型
        switch (subID)
        {
        ...
        name += Unit::localeSymbol(static_cast<UnitType>(type)) + ")";
        // 设置item标签的名字
        ...
        prefix += "|";
        prefix += Unit::getSymbol(static_cast<UnitType>(type));
        // 获取指定路径的值
        ...
        else //浮点型模式的处理
        {
            ...
            fLowMaxValue = static_cast<double>(high / scale - fStepValue);
            fHighMinValue = static_cast<double>(low / scale + fStepValue);
            config->getNumAttr(prefix + "|High", "Max", tmp);
            fHighMaxValue = static_cast<double>(tmp / scale);
            item->lower->setRange(fLowMinValue, fLowMaxValue);
            item->lower->setValue(static_cast<double>(low / scale));
            item->lower->setStep(fStepValue);
            item->upper->setRange(fHighMinValue, fHighMaxValue);
            item->upper->setValue(static_cast<double>(high / scale));
...
void ConfigEditAlarmLimitMenu::_limitChanged(QString valueStr, int id)
{
    ...
    SubParamID subID = item->sid;
    type = static_cast<int>(paramInfo.getUnitOfSubParam(subID));
    switch (subID)
    {
    case SUB_PARAM_ETCO2:
    case SUB_PARAM_FICO2:
    ...
    if (0 == id % 2)
    {
        if (scale != 1)// 保存报警参数的下限 浮点型
        {
            ...
            dmin = value + static_cast<double>(step / scale);
            item->upper->setRange(dmin, dmax);

            config->setNumValue(prefix + "|Low", static_cast<int>(value * scale));
        }
        else// 保存报警参数的下限 整型
        {
    ...
    else
    {
        if (scale != 1)// 保存报警参数的上限 浮点型
        {
            ...
            dmax = value - static_cast<double>(step / scale);
            item->lower->setRange(dmin, dmax);
            config->setNumValue(prefix + "|High", static_cast<int>(value * scale));
        }
        else// 保存报警参数的上限 整型
        {
            int value = valueStr.toInt();
