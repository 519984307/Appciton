...
public:
    ...
    WiFiProfileMenuContentPrivate(WiFiProfileMenuContent * const q_ptr):q_ptr(q_ptr){}
    void loadProfiles();
...
void WiFiProfileMenuContentPrivate::onWifiConnected(const QString &ssid)
{
    ...
    if(profiles.count() > 0 && select < profiles.count() && select >= 0)
    {
        ...
        {
            if(combos[ITEM_CBO_ACCESSPOINT_PROFILE]->currentIndex() != select + 1)  //the first item is the off option, need to skip
            {
                combos[ITEM_CBO_ACCESSPOINT_PROFILE]->blockSignals(true);
...
void WiFiProfileMenuContent::timerEvent(QTimerEvent *e)
{
    ...
    {
        if(this->isVisible() && networkManager.isWiFiConnected() && d->combos[WiFiProfileMenuContentPrivate::ITEM_CBO_ACCESSPOINT_PROFILE]->currentIndex() != 0)
        {
...
void WiFiProfileMenuContent::showEvent(QShowEvent *e)
{
    ...
    connect(d->combos[WiFiProfileMenuContentPrivate::ITEM_CBO_ACCESSPOINT_PROFILE], SIGNAL(currentIndexChanged(int)), this, SLOT(onProfileSelect(int)));
...
void WiFiProfileMenuContent::hideEvent(QHideEvent *e)
{
    Q_D(WiFiProfileMenuContent);
    d->timer.stop();
    disconnect(d->combos[WiFiProfileMenuContentPrivate::ITEM_CBO_ACCESSPOINT_PROFILE], SIGNAL(currentIndexChanged(int)), this, SLOT(onProfileSelect(int)));
