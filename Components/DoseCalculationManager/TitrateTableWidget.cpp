...
void TitrateTableWidget::layoutExec()
{
    ...
    for(int i = 0; i < TITRATE_TABLE_NR; i ++)
    {
        ...
        _paramName[i]->setFont(fontManager.textFont(fontSize));
        _paramValue[i]->setFont(fontManager.textFont(fontSize));
        _paramUnit[i]->setFont(fontManager.textFont(fontSize));
        _paramName[i]->setFixedSize(labelWidth, _itemH);
        _paramValue[i]->setFixedSize(labelWidth, _itemH);
        _paramUnit[i]->setFixedSize(labelWidth, _itemH);
...
//    _titrateTable->setRowCount(9);
    ...
    _titrateTable->setFixedHeight(250);

    _tableSet = new IButton(trs("TitrateTableSet"));
    _tableSet->setFixedSize(btnWidth, _itemH);
    _tableSet->setFont(fontManager.textFont(fontSize));
    ...
    _record->setFont(fontManager.textFont(fontSize));
    ...
    _up->setPicture(QImage("/usr/local/iPM/icons/ArrowUp.png"));
    connect(_up, SIGNAL(realReleased()), this, SLOT(_upReleased()));

    _down = new IButton();
    _down->setFixedSize(_itemH - 2, _itemH - 2);
    _down->setPicture(QImage("/usr/local/iPM/icons/ArrowDown.png"));
