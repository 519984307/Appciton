...
public:
    WavPlayerPrivate()
        :playing(false), requestStop(false), curVolume(50),
          ...
          chunk_size(0), buffer_size(0), audiobuf(NULL), mixer_handle(NULL),
          pcm_elem(NULL)
    {
        int err;
...
void WavPlayer::play(WavFile *wavfile)
{
    ...
    while(!d_ptr->requestStop)
    {
        ...
        if(size -  index < (int )d_ptr->chunk_bytes)
        {
        ...
        ret = d_ptr->pcm_write(d_ptr->audiobuf, count);
