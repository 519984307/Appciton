...
public:
    ...
    enum ScrollMetric
    {
        DragVelocitySmoothingFactor,    // qreal [0..1/s]
                                        // (complex calculation involving time) v = v_new* DASF + v_old * (1-DASF)
    ...
    void registerDebugHook(void (*callback)(void *user, const QPointF &releaseVelocity,
                                            const QPointF &position, const QPointF &overshootPosition),
                           void *user);
protected:
    QKineticScroller();
    ...
    explicit QKineticScroller(QKineticScrollerPrivate *dd);
