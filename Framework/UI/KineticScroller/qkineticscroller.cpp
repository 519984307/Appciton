...
void QKineticScroller::registerDebugHook(void (*callback)(void *user,
                                                          const QPointF &releaseVelocity,
                                                          const QPointF &position,
                                                          const QPointF &overshootPosition),
                                         void *user)
{
    Q_D(QKineticScroller);
...
void QKineticScroller::resetScrollMetrics()
{
    ...
    // QPointF(qreal(14.25 / 1000), qreal(14.25 / 1000)));
    metrics.insert(OvershootMaximumDistance, QPointF(0, 0));
    metrics.insert(DragStartDistance, qreal(2.5 / 1000));
    metrics.insert(DragStartDirectionErrorMargin, qreal(1.0 / 1000));
...
void QKineticScrollerPrivate::timerEvent(QTimerEvent *e)
{
    ...
    for (int i = 0; i < static_cast<int>(sizeof(timerevents) / sizeof(*timerevents)); ++i) {
...
bool QKineticScroller::handleInput(Input input, const QPointF &position, qint64 timestamp)
{
    ...
    for (int i = 0; i < static_cast<int>(sizeof(statechanges) / sizeof(*statechanges)); ++i) {
...
void QKineticScroller::setScrollMetric(ScrollMetric metric, const QVariant &value)
{
    ...
    case DragVelocitySmoothingFactor:
        ...
        break;
    case LinearDecelerationFactor:
        d->linearDecelerationFactor = qBound(qreal(0), value.toReal(), qreal(1));
        break;
    case ExponentialDecelerationBase:
        d->exponentialDecelerationBase = qBound(qreal(0), value.toReal(), qreal(1));
        break;
    case OvershootSpringConstant:
        d->overshootSpringConstantRoot = qSqrt(value.toReal());
        break;
    case OvershootDragResistanceFactor:
        d->overshootDragResistanceFactor = value.toReal();
        break;
    case OvershootMaximumDistance:
        d->overshootMaximumDistance = value.toPointF();
        break;
    case DragStartDistance:
        d->dragStartDistance = value.toReal();
        break;
    case DragStartDirectionErrorMargin:
        d->dragStartDirectionErrorMargin = value.toReal();
        break;
    ...
    case MaximumVelocity:
        ...
        break;
    case MaximumNonAcceleratedVelocity:
        d->maximumNonAcceleratedVelocity = value.toReal();
        break;
    ...
    case FramesPerSecond:
        d->framesPerSecond = qBound(1, value.toInt(), 100);
        break;
    case FastSwipeMaximumTime:
        d->fastSwipeMaximumTime = value.toReal();
        break;
    case FastSwipeMinimumVelocity:
        d->fastSwipeMinimumVelocity = value.toReal();
        break;
    case FastSwipeBaseVelocity:
        d->fastSwipeBaseVelocity = value.toReal();
        break;
...
void QKineticScrollerPrivate::updateVelocity(const QPointF &deltaPixelRaw, qint64 deltaTime)
{
    ...
    if (((deltaPixelRaw / qreal(deltaTime)).manhattanLength() / pixelPerMeter * 1000) > qreal(2.5))
        deltaPixel = deltaPixelRaw * qreal(2.5) * pixelPerMeter
                / 1000 / (deltaPixelRaw / qreal(deltaTime)).manhattanLength();
...
void QKineticScrollerPrivate::handleDrag(const QPointF &position, qint64 timestamp)
{
    ...
    if (axisLockThreshold) {
        ...
        if (dx || dy) {
            ...
            // qKSDebug() << "QKS::handleDrag() -- axis lock:" << alpha << " / "
            // << axisLockThreshold << "- isvertical:" << vertical << "- dx:" << dx << "- dy:" << dy;
...
bool QKineticScrollerPrivate::releaseWhileDragging(QKineticScroller::Input, const QPointF &, qint64 timestamp)
{
        ...
        (oldVelocity > fastSwipeMinimumVelocity))
    {
        ...
        releaseVelocity.setX(signX * (oldVelocity.x() + (oldVelocity.x() > 0 ?
                                                             fastSwipeVelocity.x() :
                                                             -fastSwipeVelocity.x())));
        releaseVelocity.setY(signY * (oldVelocity.y() + (oldVelocity.y() > 0 ?
                                                             fastSwipeVelocity.y() :
                                                             -fastSwipeVelocity.y())));
    }
    ...
    qKSDebug() << "QKS::releaseWhileDragging() -- velocity:"
               << releaseVelocity << "-- minimum velocity:" << minimumVelocity;
    if (overshootX || overshootY)
        setState(QKineticScroller::StateScrolling);
    else if (releaseVelocity >= minimumVelocity)
        setState(QKineticScroller::StateScrolling);
...
void QKineticScrollerPrivate::timerEventWhileScrolling()
{
    ...
    qKSDebug() << "QKS::timerEventWhileScrolling() -- DeltaPos:"
               << deltaPos << "- NewVel:" <<  newVelocity
               << "- Time:" <<  time;
...
void QKineticScrollerPrivate::setState(QKineticScroller::State newstate)
{
    ...
    case QKineticScroller::StateInactive:
        if (state == QKineticScroller::StateScrolling) {
            if (timerId) {
                ...
                qKSDebug() << "  --> state change from "
                           << stateName(state)
                           << " to " << stateName(newstate)
                           << ", but timer is not active.";
            }
        }
    ...
    case QKineticScroller::StateScrolling:
        ...
        if (state == QKineticScroller::StateDragging) {
            // TODO(chenbingyun): better calculate StartTime using the current releaseVelocity
            overshootStartTimeX = overshootStartTimeY =
                    qreal(scrollAbsoluteTimer.elapsed()) / 1000 - M_PI / (overshootSpringConstantRoot * 2);
...
void QKineticScroller::scrollTo(const QPointF &pos, int scrollTime)
{
    ...
    */
    ...
    QPointF v = (scrollDir * (d->linearDecelerationFactor / qreal(2)) *
                 qreal(time) * qreal(time) + (pos - contentPosition()) / d->pixelPerMeter);
...
void QKineticScroller::ensureVisible(const QPointF &pos, int xmargin, int ymargin, int scrollTime)
{
    ...
    qKSDebug() << "QKS::ensureVisible(" << pos << " [pix], " << xmargin
...
void QKineticScrollerPrivate::setContentPositionHelper(const QPointF &deltaPos)
{
    ...
    if (!overshootMaximumDistance.isNull()) {
        newOvershootX = qBound(-overshootMaximumDistance.x() * pixelPerMeter,
                               newOvershootX, overshootMaximumDistance.x() * pixelPerMeter);

        newOvershootY = qBound(-overshootMaximumDistance.y() * pixelPerMeter,
                               newOvershootY, overshootMaximumDistance.y() * pixelPerMeter);
    }
    ...
    if (debugHook)
        debugHook(debugHookUser, calculateVelocity(qreal(scrollAbsoluteTimer.elapsed()) / 1000),
                  newClampedPos, overshootPosition);

    qKSDebug() << "  --> new position:" << newClampedPos << "- new overshoot:" << overshootPosition <<
                  "- overshoot x/y?:" << overshootX << "/" << overshootY
               << "- scrollto x/y?:" << scrollToX << "/" << scrollToY;
}
