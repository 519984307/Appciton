...
QImage captureScreen()
{
    ...
    if (fd >= 0)
    {
        ...
        fbp = static_cast<char *>(mmap(0, screenSize, PROT_READ, MAP_SHARED, fd, 0));
...
QImage captureScreen()
{
    QPoint leftTop = windowManager.mapToGlobal(windowManager.rect().topLeft());
    QPixmap pix =  QPixmap::grabWindow(qApp->desktop()->winId(), leftTop.x(), leftTop.y(),
                                       windowManager.width(), windowManager.height());
    return pix.toImage();
}
#endif
}   // namespace Util
