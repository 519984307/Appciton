...
void BLMEDUpgradeParam::run(void)
{
    ...
    {
        serviceUpgrade.infoShow(true);
        if (_isUSBExist)
        {
            _isUSBExist = false;
            if (serviceUpgrade.isUpgrading() && !_isFinish)
            {
                TimeStop();
                serviceUpgrade.setDebugText(trs("WarningNoUSB"));
                serviceUpgrade.setDebugText(trs("UpgradeInterrupt") + "! ");
                serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
                _btnShow(true);
                if (_file.isOpen())
                {
                    _file.close();
    ...
    if (isTimeOut() && _isUSBExist)
    {
        ...
        serviceUpgrade.setDebugText(trs("UpgradeTimeout") + ", "
...
void BLMEDUpgradeParam::connectError()
{
    //    TimeStop();
    //    _btnShow(true);
    //    serviceUpgrade.setDebugText(trs("UpgradeCommunicationInterrupt"));
}
...
void BLMEDUpgradeParam::_btnShow(bool flag)
{
    ...
    {
        //退出TN3透传模式
        if (_type == UPGRADE_TYPE_TN3Daemon)
        {
            _serviceUpgradeTN3Daemon(false);
        }
        serviceUpgrade.btnShow(true);
    }
    else
    {
        serviceUpgrade.btnShow(false);
    }
}
...
void BLMEDUpgradeParam::_USBExist()
{
    ...
    else
    {
        _isUSBExist = true;
        serviceUpgrade.infoShow(false);
        serviceUpgrade.setDebugText(trs("UpgradeFindImage"));
        _fileExist();
    }
}
...
void BLMEDUpgradeParam::_fileExist()
{
    ...
    if (_type == UPGRADE_TYPE_HOST)
    {
        ...
        if (!QFile::exists(dstFile))
        {
            TimeStop();
            //导入文件不存在或文件名错误
            serviceUpgrade.setDebugText(trs("UpgradeWarningNoImportFile") + ":");
            serviceUpgrade.setDebugText(dstFile);
            return;
        }
        TimeStop();
        serviceUpgradeStart();
    ...
    else if (_type >= UPGRADE_TYPE_TE3 && _type < UPGRADE_TYPE_NR)
    {
        QString path(usbManager.getUdiskMountPoint());
        path += "/ZOLLImage/";
        QString file;

        if (!_findFile(path, QString(serviceUpgrade.convert(_type)), file))
        {
            TimeStop();
            return;
        }
        ...
        if (!_file.isOpen())
        {
            ...
            {
                serviceUpgrade.setDebugText(dstFile + " " + trs("UpgradeOpenFail"));
                TimeStop();
                return;
            }
        }
        ...
        QString str = serviceUpgrade.convert(_type);
        serviceUpgrade.setDebugText(str + " "  + trs("ModuleUpgradeStart"));
...
//    {
//        TimeStop();
//        serviceUpgrade.setDebugText(trs("MD5VerifyError"));
//        _file.close();
//        return;
//    }
...
//    else
//    {
//        serviceUpgrade.setDebugText(trs("MD5NOTMATCH"));
//        serviceUpgrade.setDebugText(trs("ORIGINALMD5") + ": "+ md5Original);
//        serviceUpgrade.setDebugText(trs("CALCULATEDMD5") + ": "+ binMD5);
//        _file.close();
//    }
...
//    debug("%s",qPrintable(strMD5));
    ...
    else
    {
        serviceUpgrade.setDebugText(trs("MD5NOTMATCH"));
        serviceUpgrade.setDebugText(trs("ORIGINALMD5") + ": "+ md5Original);
        serviceUpgrade.setDebugText(trs("CALCULATEDMD5") + ": "+ strMD5);
        _file.close();
    }
}
...
void BLMEDUpgradeParam::serviceUpgradeStart(void)
{
    ...
    if(_type == UPGRADE_TYPE_HOST)
    {
        serviceUpgrade.setDebugText(trs("SystemWillRestartDuringTheUpgradeProcess"));
        serviceUpgrade.setDebugText(trs("UpgradePleasedonotpoweroff"));
...
void BLMEDUpgradeParam::_restartTimeOut(void)
{
    if (_isUSBExist)
    {
        ...
        int ret = system("fw_setenv maspro_user_mode 1");
        ret = system("reboot");
        ret = ret;
    }
}
...
//    serviceUpgradeRead();
    ...
    else
    {
        ...
        else
        {
            ...
            _btnShow(true);
            serviceUpgrade.setDebugText(trs("UpgradeTimeout") + "! " + trs("UpgradeEnterFailure"));
            _file.close();
        }
    }
}
...
void BLMEDUpgradeParam::serviceUpgradeSend(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            _btnShow(true);
            serviceUpgrade.setDebugText(trs("UpgradeTimeout") + "! " +trs("UpgradeInitFailure"));
            _file.close();
        }
    }
}
    ...
    else
    {
        ...
        if (!_file.atEnd())
        {
            ...
            if (_fileSum > 128)
            {
                ...
                {
                    serviceUpgrade.setDebugText(trs("UpgradeFileReadError") + "! ");
                    serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
                    _btnShow(true);
                    return;
                }
            ...
            else
            {
                ...
                {
                    serviceUpgrade.setDebugText(trs("UpgradeFileReadLastError") + "! ");
                    serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
                    _btnShow(true);
                    return;
                }
...
void BLMEDUpgradeParam::unPacket(unsigned char *packet, int /*len*/, const QString &name)
{
    ...
    case UPGRADE_TYPE_TN3Daemon:
        ...
        if ((packet[0] == UPGRADE_PASSTHROUGH_MODE))
        {
            ...
            {
                debug("进入透传模式");
                if (_type == UPGRADE_TYPE_TN3Daemon && _sendThroughFlag)
                {
                    serviceUpgradeEnter();
            ...
            else
            {
                if (_sendThroughFlag)
                {
                    serviceUpgrade.setDebugText(trs("UpgradeEnterTN3DaemonModuleFail"));
                    _btnShow(true);
                    _sendThroughFlag = false;
                }
                else
    ...
    if ((packet[0] == UPGRADE_NOTIFY_ALIVE) || (packet[0] == UPGRADE_ALIVE) || (packet[0] == UPGRADE_ECG_NOTIFY_ALIVE))
    {
        ...
        if (_type == UPGRADE_TYPE_TN3Daemon)
        {
            if (_upgradeTN3DaemonFlag)
            {
                ...
                {
                    TimeStop();
                    serviceUpgrade.setDebugText(trs("UpgradePleaseUpgradeTN3ModuleFirst"));
                    _btnShow(true);
                    _file.close();
                    _upgradeTN3DaemonFlag = false;
                }
        ...
        if (packet[0] == UPGRADE_ALIVE)
        {
            if (_isFinish)
            {
                ...
                if (_numFe > 5)
                {
                    serviceUpgrade.setDebugText(trs("UpgradeFailure") + "! ");
                    serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
                    _btnShow(true);
                    _isFinish = false;
                }
        ...
        else
        {
            ...
            if (_isFinish)
            {
                ...
                {
                    TimeStop();
                    serviceUpgrade.progressBarValue((int)(_fileNum*100/((_fileSize-128)/128)));
                    debug("%s",qPrintable(name));
                    serviceUpgrade.setDebugText(trs("UpgradeSuccess") + "! ");
                    _btnShow(true);
                }
            }
        }
...
//        blmedUpgradeTime.stopFile();
    ...
    if ((packet[0] == UPGRADE_RESPONSE_NACK) || (packet[0] == UPGRADE_RESPONSE_ACK))
    {
        ...
        if (packet[1] == UPGRADE_RSP_ENTER)
        {
            ...
            {
                _btnShow(true);
                serviceUpgrade.setDebugText(trs("UpgradeEnterFailure") + "! ");
                _file.close();
            }
            else
            {
        ...
        else if (packet[1] == UPGRADE_RSP_SEND)
        {
            ...
            {
                _btnShow(true);
                serviceUpgrade.setDebugText(trs("UpgradeInitFailure") + "! ");
                _file.close();
            }
            else
            {
        ...
        else if (packet[1] == UPGRADE_RSP_READ)
        {
            ...
            {
                serviceUpgrade.setDebugText(trs("UpgradeReadFailure") + "! ");
                _file.close();
            }
            else
            {
                ...
                serviceUpgrade.setDebugText(trs("UpgradeReadSuccess") + "! ");
                serviceUpgrade.setDebugText(trs("UpgradeSoftwareNumber") + ": " + softNumber + " " +
        ...
        else if (packet[1] == UPGRADE_RSP_CLEAR)
        {
            ...
            {
                _btnShow(true);
                serviceUpgrade.setDebugText(trs("UPGRADECLEARFAIL") + "! ");
                _file.close();
            }
            else
            {
                serviceUpgrade.setDebugText(trs("UpgradeClearSuccess") + "! ");
                //                serviceUpgradeSend();
            }
        }
        return;
...
//        blmedUpgradeTime.stopFile();
        if (!isServiceUpgradeFile(packet))
        {
            TimeStop();
            _btnShow(true);
            serviceUpgrade.setDebugText(trs("UpgradeTransferError") + "! ");
            serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
        }
        else
        {
...
void BLMEDUpgradeParam::serviceUpgradeClear(void)
{
    ...
    setTimeOut(10 * 1000);
    serviceUpgrade.setDebugText(trs("UPGRADECLEARING") + "... ");
}
...
bool BLMEDUpgradeParam::_findFile(const QString &path, const QString &matchStr, QString &fileName)
{
    ...
    if(!dir.exists())
    {
        serviceUpgrade.setDebugText(trs("UpgradeWarningNoImportFolder") + ":");
        serviceUpgrade.setDebugText(path);
        return false;
    }
...
//        if (list[i].fileName().indexOf(matchStr) != -1)
    ...
    serviceUpgrade.setDebugText(trs("UpgradeWarningNoImportFile") + ":");
    serviceUpgrade.setDebugText(path + matchStr);
    return false;
}
...
void BLMEDUpgradeParam::isServiceUpgradeFinish(unsigned char *packet)
{
    ...
    case 0x00:
        ...
        break;
    case 0x01:
        serviceUpgrade.setDebugText(trs("UPGRADEBLMLOADERLOAD") + "! ");
    ...
    case 0x02:
        ...
        serviceUpgrade.setDebugText(trs("UPGRADEIMAGETRANSFERRING") + "...");
        sendFile();
        break;
    case 0x03:
        serviceUpgrade.setDebugText(trs("UPGRADESIGNFAIL") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x04:
    ...
    case 0x06:
        //软件镜像名称不匹配
        serviceUpgrade.setDebugText(trs("UPGRADEIMAGEISWRONG") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x07:
        //硬件版本不匹配
        serviceUpgrade.setDebugText(trs("UPGRADEHARDWAREVERSIONDOESNOTMATCH") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x08:
        //写设备属性（软件镜像名称+硬件版本）到FLASH失败
        serviceUpgrade.setDebugText(trs("UPGRADEPROPERTYWRITEFAILED") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x09:
        //文件片段的序号错误
        serviceUpgrade.setDebugText(trs("UpgradeFailure") + "! ");
        serviceUpgrade.setDebugText(trs("UPGRADEFILEORDERNUMBERERROR") + "! ");
        serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x0A:
        //写文件片段到FLASH失败
        serviceUpgrade.setDebugText(trs("UpgradeFailure") + "! ");
        serviceUpgrade.setDebugText(trs("UPGRADEFILEFLASHFAIL") + "! ");
        serviceUpgrade.setDebugText(trs("UpgradePowerOffRestart") + "! ");
        _btnShow(true);
        _file.close();
        break;
    case 0x0B:
        debug("升级完成");
        if (_type == UPGRADE_TYPE_nPMBoard)
        {
            serviceUpgrade.setDebugText(trs("UpgradeSuccess") + "! ");
            serviceUpgrade.setDebugText(trs("SystemWillRestart"));
            serviceUpgrade.setDebugText(trs("UpgradePleasedonotpoweroff"));
        }
        else
        {
