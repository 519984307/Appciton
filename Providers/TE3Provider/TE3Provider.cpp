...
void TE3Provider::_handleECGRawData(const unsigned char *data, unsigned len)
{
    ...
    for(unsigned j = 5; j < len ; j += 28)
    {
        ...
        {
            v = (data[i + 1] << 8) | (data[i]);
            rawData[rawDataIndex++] = (int)((v & 0x8000) ? (-((~(v - 1)) & 0xffff)) : v);
        }
        ...
        ecgParam.updateWaveform(leadData, leadOff, ipaceMark, epaceMark, rMark);
...
void TE3Provider::handlePacket(unsigned char *data, int len)
{
    ...
    switch (data[0])
    {
        ...
        case TE3_RSP_SELFTEST_RESULT:
        {
            if (0 == data[1])
            {
                ECGParam::selfTestResult |= MAJOR_SAMPLE_MODULE_INIT_FAILED;
            }

            if (0 == data[2])
            {
                ECGParam::selfTestResult |= MINOR_SAMPLE_MODULE_INIT_FAILED;
            }
            break;
        }
        ...
        case TE3_CYCLE_VFVT:
        {
            ...
            ecgParam.updateVFVT((bool)vfvtValue);
            break;
        }
        ...
        case TE3_CYCLE_STORE:
        {
            ...
            if (enable)
            {
                rawDataCollection.pushData("BLM_TE3", data,len);
            }
            break;
        }
        ...
        case TE3_WARNING_ERROR_CODE:
        {
            ...
            switch (data[1])
            {
                ...
                case 0x02:
                    ...
                    errorStr.sprintf("Major Collection Module %d Packet Loss.\r\n", num);
                    break;

                case 0x03:
                    num = ((data[5] << 24) | (data[4] << 16) | (data[3] << 8) | data[2]);
                    errorStr.sprintf("Minor Collection Module %d Packet Loss.\r\n", num);
                    break;

                case 0x04:
                    num = ((data[5] << 24) | (data[4] << 16) | (data[3] << 8) | data[2]);
                    errorStr.sprintf("Major Collecting %d Error Data Frames.\r\n", num);
                    break;

                case 0x05:
                    num = ((data[5] << 24) | (data[4] << 16) | (data[3] << 8) | data[2]);
                    errorStr.sprintf("Minor Collecting %d Error Data Frames.\r\n", num);
                    break;
...
void TE3Provider::disconnected(void)
{
    ...
    systemManager.setPoweronTestResult(TE3_MODULE_SELFTEST_RESULT, SELFTEST_FAILED);
}
